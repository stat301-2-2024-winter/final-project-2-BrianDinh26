...
With this model in mind, one aspect to explore in the future with this dataset is increasing the minimal node size range in order to see at what point having a higher number range leaves diminishing returns relative to how long it takes to compute the model. Additionally, there is room to compare these used car prices in comparison to the new car market and see if the newer car prices can predict how used car prices change.