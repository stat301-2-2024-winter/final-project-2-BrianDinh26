...
# create kitchen sink tables and comparisons
table_null <- null_fit |>
  show_best("rmse") |>
  slice_min(mean) |>
  mutate(model = "null",
...
# kitchen sink table
kitchen_sink_table <- bind_rows(table_null,
                                table_olr,
                                table_rf,
                                table_bt,
                                table_elastic,
...
#combine all models into a table (feature engineered recipes)
...
# best parameters for feature engineered recipes
knn_select_2 <- select_best(knn_fit_eng, metric = "rmse") |>
  rename(Config = .config,
         Neighbors = neighbors)

elastic_select_2 <-
  select_best(tuned_elastic_eng, metric = "rmse") |>
  rename(Penalty = penalty,
         Mixture = mixture,
         Config = .config)
...
# find best model out of both recipes
comparison_table <-
  bind_rows(
    table_null,
    table_null_2,
...
save(
  ...
  bt_select_2,
  rf_select_2,
  file = here("figures/best_hyperparameters_2.rda")
)
save(
  knn_select_1,
  elastic_select_1,
  bt_select_1,
  rf_select_1,
  file = here("figures/best_hyperparameters_1.rda")
)
save(
  engineered_final_table,
  comparison_table,
  kitchen_sink_table,
  file = here("figures/rmse_tables.rda")
)
